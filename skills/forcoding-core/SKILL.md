...
# ForCoding Core v2.6.2 — Design Thinking Workflow
...
(Empathize)          (Define)           (Ideate → Prototype)       (Test)
    │                    │                    │                        │
  Insight           Discovery (v2.7)  Define → Explore → Plan    Prototype → Build → Verify
```
...
| Discovery | Discovery 阶段 (v2.7) | Define — define the right problem | 三级分流: FAST-TRACK/STANDARD/FULL + USER_GATE |
...
| Auto-inject manifest EXECUTED? | ⚠️ | Auditor verifies ≥ 70% manifest items were actually executed (v2.6) |
| Visual Lead Protocol? | ⚠️ | UI tasks → CSS/HTML by SINGLE builder (v2.6 新增) |
| Visual lead identified? | ⚠️ | UI tasks → One Builder designated as Visual Lead in prompt (v2.6) |
| Polish round mandatory? | ✅ | UI tasks → Round 2 is NOT optional, is STRUCTURAL (v2.6 强制执行) |
...
| Visual review criteria injected? | ⚠️ | UI tasks → forcoding-visual-review 26 criteria in prompt (v2.6) |
| Context Drop Protection? | ⚠️ | Manifest extracted BEFORE ctx_reduce of skill content (v2.6 新增) |
| Reference analysis done? | ⚠️ | standard+ UI tasks → Reference analysis injected to Builder (改进6) |
| **Registration integrity?** | ⚠️ | All subagent .md files synced to BOTH ~/.config/opencode/forcoding/agents/ AND ~/.config/opencode/agents/ (v2.6.3 新增 — 2026-06-08 P0事故: forcoding-designer 因缺失全局 agents 注册不可dispatch) |
| **Discovery phase done?** | ✅ | standard+ tasks → discovery.md exists + USER_GATE passed (v2.7 I3) |
| **Fullstack API contract?** | ⚠️ | project_type=fullstack → Designer spec includes API Contract section (v2.8 I9) |
| **Gate files created?** | ⚠️ | each completed stage → .approved file created via task(general) (v2.8 I11) |
| **Workflow template?** | ✅ | reference docs/forcoding/workflows/template.yaml (v2.8 I12) |
...
### ⛔ Context Drop Protection（v2.6 + v2.6.3 增强 — 防止 ctx_reduce 摧毁技能知识）
...
### ⛔ Visual Lead Protocol (v2.6 新增 — 结构门禁)
...
> **🔒 Pre-Builder Gate (v2.6 结构门禁 — 编排器手动执行)**
>
...
## 📐 Section Marker Standard (v2.6.3 — Same-File Parallelism)
...
#### 2. UI-UX-Pro-Max Design System Generator (数据驱动 — v2.6)

**运行此脚本替代人工选色/选参**。ui-ux-pro-max v2.5 (88.7k ⭐) 提供 161 条行业推理规则 + 67 种 UI 风格 + 161 套调色板。
...
### Design System Seed (改进17: 数据驱动设计 — v2.6 ui-ux-pro-max v2.5 集成)

ui-ux-pro-max v2.5 (88.7k ⭐) 的 Design System Generator 是 Phase 0 的**首选数据源**。161 条推理规则 + 67 种 UI 风格自动生成设计系统：
...
### Phase 1.5: Reference Analysis (改进6: 参考基准注入 — v2.6 DESIGN.md 库)
...
### ⛔ Phase 3: Visual Lead + Polish Round (v2.6 改 — 视觉主设 + 两轮强制)
...
- 🔴 NEVER split CSS/HTML across multiple builders for UI tasks — Visual Lead Protocol (v2.6)
...
- 🔴 NEVER dispatch UI Builder without DESIGN.md tokens (Phase 0 output) in prompt (v2.6)
- 🔴 NEVER skip DESIGN.md reference library for standard+ UI tasks — Builder needs token-level reference (v2.6)
- 🔴 NEVER pass session history to sub-agents — build fresh, precisely crafted context per agent (v2.6)
- 🔴 NEVER pause between tasks in autonomous mode unless truly BLOCKED — "progress updates" waste user time (v2.6)
- 🔴 NEVER skip Plan Self-Review before output — check spec coverage, placeholders, type consistency (v2.6)
- 🔴 NEVER claim VALIDATED in Auditor without fresh evidence (v2.6)
...
### Part B: 🔴 Dispatch Verification (v2.7 — MANDATORY for standard+)
...
### Part C: HITL Approval Tiers (v2.7, I7)
...
5. **⚠️ Registration integrity audit**: verify every `mode: subagent` agent file exists in BOTH `~/.config/opencode/forcoding/agents/` AND `~/.config/opencode/agents/`. File sizes must match. (v2.6.3 新增 — 2026-06-08 P0 事故后强制)
